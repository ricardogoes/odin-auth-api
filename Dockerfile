...
COPY src/Odin.Auth.Infra.Data/*.csproj ./src/Odin.Auth.Infra.Data/
...
RUN dotnet nuget add source -u "ricardogoes" -p "${GH_PAT}" --store-password-in-clear-text --name github "https://nuget.pkg.github.com/ricardogoes/index.json" \
    && dotnet restore
...
RUN dotnet build "./src/Odin.Auth.Api/Odin.Auth.Api.csproj" -c Release -o /app/build --no-restore
